...
> 审计日期：2026-08-22
> 审计基线：`cli-wechat-bridge` 1.1.5 及 2026-08-22 的 `main`
> 文档用途：核查哪些入口属于核心功能、兼容入口、高级调试、历史遗留、可选便利层或内部协议。
> 重要：本文同时记录审计事实和已经确认的命令面决策；未列入“已确认决策”的候选项仍不代表授权删除。
> 当前版本的企业微信命令和双通道行为以 [README](../README.md) 与 [运行配置](configuration.md) 为准；本文保留为历史审计快照。
...
| 是否有人期望从微信执行 `/resume`？ | 是 | Codex、Claude、OpenCode、Pi 均已接入 | 保留并强化为统一会话控制命令 |
...
5. `/resume` 已不再是无效公开命令：Codex、Claude、OpenCode、Pi 均具备双向执行路径，并按各自原生控制面确认可见会话切换。
